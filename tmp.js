...
function fmtTime(sec){ sec = Math.max(0, Math.floor(sec)); return String(Math.floor(sec/60)).padStart(2,'0') + ':' + String(sec%60).padStart(2,'0'); }
  ...
  function refreshTop(){
    ...
    const ps = getPlanState();
    const bpmRaw = state.ble.current.bpm;
    const bpmSmooth = avgRecent(4000);
    const bpmDisplay = Number.isFinite(bpmSmooth) ? bpmSmooth : bpmRaw;
    safeSetText('hrBig', bpmDisplay == null ? '--' : Number(bpmDisplay).toFixed(1));
    const hrBig = $('hrBig');
    if (ps.bpmTarget){
      safeSetText('hrTarget', ps.bpmTarget.min + '–' + ps.bpmTarget.max);
      if (hrBig) hrBig.className = 'metricBig ' + (bpmDisplay == null ? 'hrNeutral' : (bpmDisplay >= ps.bpmTarget.min && bpmDisplay <= ps.bpmTarget.max ? 'hrGood' : 'hrBad'));
    } else {
    ...
    if (test){
      ...
      const rawDur = testWin ? fmtShortDuration(testWin.end - testWin.start) : '?';
      const testDur = rawDur.endsWith(':00') ? rawDur.replace(':00','min') : rawDur;
      const testSeg = testWin ? String(testWin.seg||'').toUpperCase() : '?';
      if (testBadge) testBadge.textContent = 'TEST · ' + testSeg + ' · ' + testDur;
      if (testWin && psSeg === testSeg) testPhaseBadge && (testPhaseBadge.textContent = 'EN CURSO · ' + testSeg + ' · ' + testDur);
      else if (testWin && ps && ps.t >= testWin.end) testPhaseBadge && (testPhaseBadge.textContent = 'FIN TEST · ' + testSeg + ' · ' + testDur);
      else testPhaseBadge && (testPhaseBadge.textContent = 'ACTIVO · ' + testSeg + ' · ' + testDur);
    }
  }
  ...
  function setAlertBannerMessage(box, msg, cls){
    ...
    const sep = '   │   ';
    inner.textContent = [msg, msg, msg].join(sep);
    inner.className = 'alertTickerText ' + (cls || 'info');
    inner.style.animationDuration = '24s';
  }
  ...
  function refreshSessionAlert(){
    ...
    const nowClock = getCurrentRealClock();
    const hrAgeMs = state.ble.current.receivedAt ? (Date.now() - state.ble.current.receivedAt) : Infinity;
    const hrWarn = (state.ble.connectionState !== 'conectado' || !Number.isFinite(state.ble.current.bpm) || hrAgeMs >= 5000);
    if (!state.plan.rows.length) {
      cls = 'info';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ Carga un plan para empezar';
    } else if (hrWarn) {
      cls = 'warn';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ ⚠️ PULSÓMETRO ' + (state.ble.connectionState !== 'conectado' ? 'DESCONECTADO' : (Number.isFinite(state.ble.current.bpm) ? 'INESTABLE' : 'SIN SEÑAL')) + ' │ Tramo ' + (ps.seg || '--') + ' │ Nivel ' + (ps.level ?? '--');
    } else if (testWindow && ps.seg === String(testWindow.seg||'').toUpperCase()) {
      cls = 'warn';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ 🧪 TEST ' + testWindow.seg + ' EN CURSO │ Δ tramo ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal │ Δ total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal';
      body.classList.add('alert-test');
    } else if (overdueWaterIdx !== -1) {
      cls = 'warn';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ 💧 AGUA AHORA │ quedan ' + fmtSpeechDuration(Math.max(0, Math.ceil((state.plan.duration || 0) - t))) + ' de elíptica';
      body.classList.add('alert-water');
    } else if (nextWaterIdx !== -1 && (state.plan.waters[nextWaterIdx] - t) <= 30) {
      cls = 'info';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ 💧 Agua en ' + fmtSpeechDuration(state.plan.waters[nextWaterIdx] - t) + ' │ prepara dos o tres sorbos';
      body.classList.add('alert-water');
    } else if (secsToSegmentEnd != null && secsToSegmentEnd <= 10) {
      cls = 'warn';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ ⏭ Cambio en ' + fmtSpeechDuration(secsToSegmentEnd) + ' │ próximo nivel ' + nextLevelAfter(t) + ' │ Δ tramo ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal';
      body.classList.add('alert-change');
    } else if (Math.abs(delta) >= 10) {
      cls = 'err';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ ⚠️ Δ total alto ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal │ Δ tramo ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal';
    } else if (Math.abs(delta) >= 5) {
      cls = 'warn';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ ⚠️ Δ total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal │ Δ tramo ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal';
    } else if (state.plan.running) {
      cls = 'ok';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ Tramo ' + (ps.seg || '--') + ' │ Nivel ' + (ps.level ?? '--') + ' │ Δ tramo ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal │ Δ total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal';
    } else if ((state.plan.sessionLog || []).length) {
      cls = 'info';
      msg = '🕒 ' + nowClock + ' │ 🏁 Fin ' + finishEta + ' │ Sesión pausada │ Tramo ' + (ps.seg || '--') + ' │ Δ total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal';
    }
    box.className = 'alertBanner ' + cls;
    setAlertBannerMessage(box, msg, cls);
    box.title = msg;
  }
