...
(() => {
  ...
  function drawCompareChart(){
    ...
    const cssH = Math.max(180, Math.round(cv.clientHeight || 220));
    ...
    const pad = { l: 52, r: 18, t: 16, b: 28 };
    const plotW = Math.max(10, cssW - pad.l - pad.r);
    const plotH = Math.max(10, cssH - pad.t - pad.b);
    const nowT = machineElapsed();
    const totalT = Math.max(1, state.plan.duration || 1);
    const windowSec = Math.min(totalT, Math.max(120, Math.ceil(plotW / 5)));
    const startT = Math.max(0, nowT - windowSec);
    const endT = Math.min(totalT, Math.max(startT + 1, startT + windowSec));
    const series = [];
    const step = Math.max(1, Math.ceil((endT - startT) / Math.max(40, Math.floor(plotW / 6))));
    for (let t = startT; t <= endT + 0.0001; t += step){
      const tt = Math.min(endT, t);
      series.push({ t:tt, plan: shownKcalAtTime(tt), real: shownRealKcalAtTime(tt) });
    }
    if (!series.length) series.push({ t:0, plan:0, real:0 });
    const nowPlan = shownKcalAtTime(nowT);
    const nowReal = shownRealKcalAtTime(nowT);
    const vals = series.flatMap(p => [p.plan, p.real]).filter(v => Number.isFinite(v));
    vals.push(nowPlan, nowReal);
    let yMin = Math.min(...vals), yMax = Math.max(...vals);
    const spread = Math.max(2, yMax - yMin);
    const padY = Math.max(1.5, spread * 0.22);
    yMin -= padY;
    yMax += padY;
    if (Math.abs(yMax - yMin) < 4){ yMin -= 2; yMax += 2; }

    const xFor = t => pad.l + ((t - startT) / Math.max(1e-9, endT - startT)) * plotW;
    ...
    const nowX = xFor(nowT), yPlan = yFor(nowPlan), yReal = yFor(nowReal);
    ...
    if (legend) legend.textContent = 'Plan ' + num(nowPlan,1) + ' · Real ' + num(nowReal,1) + ' · Ventana ' + fmtTime(startT) + '–' + fmtTime(endT);
  }
  ...
  function compareWithPrevious(){
    try{
      ...
      const txt = `Comparación con última · plan ${num(currKPlan,1)} vs ${num(prevKPlan,1)} · real ${num(currKReal,1)} vs ${num(prevKReal,1)} · FC media ${num(currAvg,1)} vs ${num(prevAvg,1)}`;
  ...
  function refreshSessionAlert(){
    ...
    const t = machineElapsed();
    const ps = getPlanState();
    const kPlan = shownKcalAtTime(t);
    const kReal = shownRealKcalAtTime(t);
    const delta = Number((kReal - kPlan).toFixed(1));
    ...
    if (!state.plan.rows.length) {
      ...
      cls = 'warn';
      msg = '💧 Toca agua ahora · toma pendiente en ' + fmtTime(state.plan.waters[overdueWaterIdx]) + '.';
      body.classList.add('alert-water');
    } else if (nextWaterIdx !== -1 && (state.plan.waters[nextWaterIdx] - t) <= 30) {
      cls = 'info';
      msg = '💧 Agua en ' + fmtTime(state.plan.waters[nextWaterIdx] - t) + '.';
      body.classList.add('alert-water');
    } else if (secsToSegmentEnd != null && secsToSegmentEnd <= 10) {
      cls = 'warn';
      msg = '⏭ Cambio de tramo en ' + fmtTime(secsToSegmentEnd) + ' · próximo nivel ' + nextLevelAfter(t) + '.';
      body.classList.add('alert-change');
    } else if (Math.abs(delta) >= 10) {
      cls = 'err';
      msg = '⚠️ Desvío alto de kcal: ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal.';
    } else if (Math.abs(delta) >= 5) {
      cls = 'warn';
      msg = '⚠️ Desvío de kcal: ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal.';
    } else if (state.plan.running) {
      cls = 'ok';
      msg = 'Sesión corriendo · tramo ' + (ps.seg || '--') + ' · nivel ' + (ps.level ?? '--') + ' · desvío ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal.';
    } else if ((state.plan.sessionLog || []).length) {
      cls = 'info';
      msg = 'Sesión pausada · tramo ' + (ps.seg || '--') + ' · nivel ' + (ps.level ?? '--') + '.';
    }
  ...
  function refreshStatusStrip(){
    ...
    setChip('chipHr', Number.isFinite(state.ble.current.bpm) && hrAgeMs < 5000 ? 'ok' : 'bad', 'Pulso', Number.isFinite(state.ble.current.bpm) ? ('Pulso: ' + Math.round(state.ble.current.bpm) + ' bpm') : 'Sin señal de pulso');
  ...
  function runStartupDiagnostics(){
    ...
    const fnMap = {withTapGuard, bindAction, drawCompareChart, refreshAll, toggleRun, applyImport, previewImport, normalizeCurrentImport, refreshStatusStrip, renderHistory, refreshSessionAlert, exportTramoCsv, updateAppStatus}; let ok=0, fail=0;
  ...
  bind(); runStartupDiagnostics(); registerPWA(); updateSavedSessionInfo(); updateExtraSavedInfo(); renderHistory(); updateAppStatus();
