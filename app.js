...
function fmtTime(sec){ sec = Math.max(0, Math.floor(sec)); return String(Math.floor(sec/60)).padStart(2,'0') + ':' + String(sec%60).padStart(2,'0'); }
  ...
  function setAlertBannerMessage(box, msg){
    ...
    if (!inner){
      box.innerHTML = '<div class="alertTickerWrap"><div id="sessionAlertText" class="alertTickerText"></div></div>';
  ...
  function refreshSessionAlert(){
    ...
    if (!state.plan.rows.length) {
      ...
      cls = 'warn';
      msg = '🧪 Test en curso · tramo ' + testWindow.seg + ' · desvío tramo ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal · desvío total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal.';
      body.classList.add('alert-test');
    } else if (overdueWaterIdx !== -1) {
      cls = 'warn';
      msg = '💧 Toca agua ahora · toma pendiente en ' + fmtTime(state.plan.waters[overdueWaterIdx]) + ' · quedan ' + fmtSpeechDuration(Math.max(0, Math.ceil((state.plan.duration || 0) - t))) + ' de elíptica.';
      body.classList.add('alert-water');
    } else if (nextWaterIdx !== -1 && (state.plan.waters[nextWaterIdx] - t) <= 30) {
      cls = 'info';
      msg = '💧 Agua en ' + fmtSpeechDuration(state.plan.waters[nextWaterIdx] - t) + ' · prepara dos o tres sorbos.';
      body.classList.add('alert-water');
    } else if (secsToSegmentEnd != null && secsToSegmentEnd <= 10) {
      cls = 'warn';
      msg = '⏭ Cambio de tramo en ' + fmtSpeechDuration(secsToSegmentEnd) + ' · próximo nivel ' + nextLevelAfter(t) + ' · tramo ' + (ps.seg || '--') + ' va ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal.';
      body.classList.add('alert-change');
    } else if (Math.abs(delta) >= 10) {
      cls = 'err';
      msg = '⚠️ Desvío total alto: ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal · desvío de tramo: ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal.';
    } else if (Math.abs(delta) >= 5) {
      cls = 'warn';
      msg = '⚠️ Desvío total: ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal · tramo: ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal.';
    } else if (state.plan.running) {
      cls = 'ok';
      msg = 'Sesión corriendo · tramo ' + (ps.seg || '--') + ' · nivel ' + (ps.level ?? '--') + ' · tramo ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal · total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal.';
    } else if ((state.plan.sessionLog || []).length) {
      cls = 'info';
      msg = 'Sesión pausada · tramo ' + (ps.seg || '--') + ' · nivel ' + (ps.level ?? '--') + ' · total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal.';
    }
