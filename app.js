...
(() => {
  ...
  const state = {
    ...
    settings: { browserNotify:false, voiceAlerts:true, beepAlerts:true, voiceName:'', voiceVolume:1, voiceRate:1, alertKinds:{segmentPre:true, segmentNow:true, waterPre:true, waterNow:true, segRemain:true, allRemain:true, hr:true, kcal:true} },
    alerts: { fired:{}, queue:[], busy:false, audioCtx:null, lastHrWarnAt:0, lastKcalWarnAt:0 }
  };
  ...
  async function sendBrowserNotification(title, body, tag){
    ...
    try{ new Notification(title, { body, tag, renotify:false, silent:false, requireInteraction:false }); log('Notificación enviada · ' + title + ' · ' + body, 'ok'); return true; }catch(err){ log('No se pudo lanzar notificación: ' + ((err&&err.message)||err), 'warn'); return false; }
  }
...
function queueSegmentCues(){
  ...
  if (alertKindEnabled('segmentNow') && prev && prev.seg !== current.seg && current.start > 0){
    const text = (current.isTest ? 'Test ahora. ' : 'Cambio ahora. ') + (current.level > prev.level ? 'Subida a ' : (current.level < prev.level ? 'Bajada a ' : 'Cambio a ')) + 'nivel ' + current.level + '. Tramo ' + current.seg + '. Quedan ' + fmtSpeechDuration(Math.ceil(current.end - t)) + ' de este tramo. Fin prevista ' + getPredictedEndClock() + '.';
  ...
  if (alertKindEnabled('waterNow')){
    state.plan.waters.forEach((w, idx) => {
      if (!state.plan.waterTaken[idx] && prevT < w && t >= w){
        const text = 'Agua ahora. Dos o tres sorbos y vuelve rápido a la cadencia. Quedan ' + fmtSpeechDuration(remainAll) + ' de elíptica. Fin prevista ' + getPredictedEndClock() + '.';
  ...
  if (alertKindEnabled('allRemain') && Math.floor(t) > 0 && prevRemainAll !== remainAll && remainAll > 0 && remainAll % 300 === 0){
    enqueueCue({ key:'allremain-' + remainAll, type:'speech', text:'Quedan ' + fmtSpeechDuration(remainAll) + ' para terminar la elíptica. Fin prevista ' + getPredictedEndClock() + '.' });
  }
}
...
function fmtTime(sec){ sec = Math.max(0, Math.floor(sec)); return String(Math.floor(sec/60)).padStart(2,'0') + ':' + String(sec%60).padStart(2,'0'); }
  ...
  function getPredictedEndClock(){
    try{
      const remainingRealSec = Math.max(0, Number(calcLiveDerivedMetrics().remainingRealSec || 0));
      return fmtClockHM(new Date(Date.now() + remainingRealSec * 1000));
    }catch(_){ return '--:--'; }
  }

  function getCurrentRealClock(){
    return fmtClockHM(new Date());
  }
  ...
  function refreshBle(){
    ...
    $('bleBpmBig').textContent = c.bpm == null ? '--' : Number(c.bpm).toFixed(1);
  ...
  document.addEventListener('visibilitychange', async () => {
    ...
    if (document.visibilityState === 'hidden'){
      ...
      if (state.plan.running){ log('App en segundo plano · sesión guardada para recuperación', 'warn'); }
    }
  });
    ...
    try{
      const payload = {
        ...
        plan: {
          ...
          runningAtSave: !!state.plan.running,
          t0WallIso: state.plan.t0WallIso || null,
  ...
  function maybePersistLiveSession(force=false){
    const now = Date.now();
    if (!force && now - (state.plan._lastPersistAt || 0) < 1000) return;
  ...
  function loadLiveSession(){
    try{
      ...
      state.plan.t0WallIso = payload.plan.t0WallIso || null;
      state.plan._runningAtSave = !!payload.plan.runningAtSave;
      state.plan._savedAtIso = payload.savedAt || null;
      state.plan._resumeGapSec = 0;
      if (state.plan._runningAtSave && payload.plan.lastWallClockMs){
        const gapSec = Math.max(0, Math.round((Date.now() - Number(payload.plan.lastWallClockMs || 0)) / 1000));
        state.plan._resumeGapSec = gapSec;
        if (gapSec > 0){
          state.plan.baseMachineSec = clamp((state.plan.baseMachineSec || 0) + gapSec, 0, state.plan.duration || 0);
          log('Sesión recuperada tras cierre/minimizado · avance estimado +' + fmtTime(gapSec), 'warn');
        }
      }
  ...
  function buildFinalSummary(){
    ...
    rows.forEach(r => {
      if (!r.level) return;
      byLevel[r.level] = byLevel[r.level] || { secs:0, kcalPlanStart:null, kcalPlanEnd:0, kcalRealStart:null, kcalRealEnd:0, bpm:[] };
      byLevel[r.level].secs += 1;
      if (byLevel[r.level].kcalPlanStart == null) byLevel[r.level].kcalPlanStart = Number(r.kcalPlan || 0);
      byLevel[r.level].kcalPlanEnd = Number(r.kcalPlan || 0);
      const realVal = Number((r.kcalReal ?? r.kcalPlan) || 0);
      if (byLevel[r.level].kcalRealStart == null) byLevel[r.level].kcalRealStart = realVal;
      byLevel[r.level].kcalRealEnd = realVal;
    ...
    const effLines = Object.keys(byLevel).sort((a,b)=>Number(a)-Number(b)).map(level => {
      ...
      return `- nivel ${level}: plan ${num(kcalPlan,1)} kcal | real ${num(kcalReal,1)} kcal | plan ${num(kcalPlan/mins,2)} kcal/min | real ${num(kcalReal/mins,2)} kcal/min | bpm medio ${num(bpm,1)} | relación plan ${num(effPlan,4)} | relación real ${num(effReal,4)}`;
    }).join('\n');
    const summary = [
  ...
  function setAlertBannerMessage(box, msg){
    ...
    if (!inner){
      box.innerHTML = '<div id="realNowBanner" class="alertFixedLeft">--:--</div><div class="alertTickerWrap"><div id="sessionAlertText" class="alertTickerText"></div></div><div id="endEtaBanner" class="alertFixedRight">Fin --:--</div>';
      inner = $('sessionAlertText');
    }
    if (!inner) return;
    const nowEl = $('realNowBanner');
    const etaEl = $('endEtaBanner');
    if (nowEl) nowEl.textContent = getCurrentRealClock();
    if (etaEl) etaEl.textContent = 'Fin ' + getPredictedEndClock();
    inner.textContent = msg;
    const secs = Math.max(12, Math.min(34, Math.ceil(String(msg||'').length / 5)));
  ...
  function refreshSessionAlert(){
    ...
    body.classList.remove('alert-change','alert-water','alert-test');
    let cls = 'info';
    let msg = 'Sin alertas críticas.';
    const t = machineElapsed();
    const ps = getPlanState();
    const m = calcLiveDerivedMetrics();
    ...
    const finishEta = getPredictedEndClock();
    const hrAgeMs = state.ble.current.receivedAt ? (Date.now() - state.ble.current.receivedAt) : Infinity;
    if (!state.plan.rows.length) {
      cls = 'info';
      msg = 'Carga un plan para empezar. Hora real ' + getCurrentRealClock() + ' · fin prevista ' + finishEta + '.';
    } else if (state.plan.running && (state.ble.connectionState !== 'conectado' || !Number.isFinite(state.ble.current.bpm) || hrAgeMs >= 5000)) {
      cls = 'warn';
      msg = '⚠️ Problema de conexión con pulsómetro · estado BLE ' + state.ble.connectionState + ' · señal ' + (Number.isFinite(state.ble.current.bpm) ? 'inestable' : 'sin pulso') + ' · fin prevista ' + finishEta + '.';
    } else if (testWindow && ps.seg === String(testWindow.seg||'').toUpperCase()) {
      cls = 'warn';
      msg = '🧪 Test en curso · tramo ' + testWindow.seg + ' · desvío tramo ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal · desvío total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal · fin prevista ' + finishEta + '.';
      body.classList.add('alert-test');
    } else if (overdueWaterIdx !== -1) {
      cls = 'warn';
      msg = '💧 Toca agua ahora · toma pendiente en ' + fmtTime(state.plan.waters[overdueWaterIdx]) + ' · quedan ' + fmtSpeechDuration(Math.max(0, Math.ceil((state.plan.duration || 0) - t))) + ' de elíptica · fin prevista ' + finishEta + '.';
      body.classList.add('alert-water');
    } else if (nextWaterIdx !== -1 && (state.plan.waters[nextWaterIdx] - t) <= 30) {
      cls = 'info';
      msg = '💧 Agua en ' + fmtSpeechDuration(state.plan.waters[nextWaterIdx] - t) + ' · prepara dos o tres sorbos · fin prevista ' + finishEta + '.';
      body.classList.add('alert-water');
    } else if (secsToSegmentEnd != null && secsToSegmentEnd <= 10) {
      cls = 'warn';
      msg = '⏭ Cambio de tramo en ' + fmtSpeechDuration(secsToSegmentEnd) + ' · próximo nivel ' + nextLevelAfter(t) + ' · tramo ' + (ps.seg || '--') + ' va ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal · fin prevista ' + finishEta + '.';
      body.classList.add('alert-change');
    } else if (Math.abs(delta) >= 10) {
      cls = 'err';
      msg = '⚠️ Desvío total alto: ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal · desvío de tramo: ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal · fin prevista ' + finishEta + '.';
    } else if (Math.abs(delta) >= 5) {
      cls = 'warn';
      msg = '⚠️ Desvío total: ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal · tramo: ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal · fin prevista ' + finishEta + '.';
    } else if (state.plan.running) {
      cls = 'ok';
      msg = 'Sesión corriendo · tramo ' + (ps.seg || '--') + ' · nivel ' + (ps.level ?? '--') + ' · tramo ' + (segDelta > 0 ? '+' : '') + num(segDelta,1) + ' kcal · total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal · fin prevista ' + finishEta + '.';
    } else if ((state.plan.sessionLog || []).length) {
      cls = 'info';
      msg = 'Sesión pausada · tramo ' + (ps.seg || '--') + ' · nivel ' + (ps.level ?? '--') + ' · total ' + (delta > 0 ? '+' : '') + num(delta,1) + ' kcal · fin prevista ' + finishEta + '.';
    }
    box.className = 'alertBanner ' + cls;
    setAlertBannerMessage(box, msg);
    box.title = msg;
  }
  ...
  function runStartupDiagnostics(){
    log('[STARTUP] Inicio comprobación ' + APP_VERSION, 'ok'); log('[STARTUP] Modo robusto de recuperación activa · guardado al ocultar/cerrar · reanudación estimada disponible', 'ok');
  ...
  function bind(){
    ...
    bindAction('installBtn','install',promptInstall); bindAction('refreshAppBtn','refreshApp',updateAppNow); bindAction('exportSessionBtn','exportSession',exportSessionLog); bindAction('exportMinuteBtn','exportMinute',exportMinuteLog); bindAction('resumeSessionBtn','resumeSession',loadLiveSession); bindAction('finalSummaryBtn','finalSummary',()=>{ buildFinalSummary(); buildTramoSummary(); compareWithPrevious(); }); bindAction('exportSessionJsonBtn','exportJson',exportSessionJson); bindAction('compareLastBtn','compareLast',()=>{ $('finalSummaryBox').value += '\n\n' + compareWithPrevious(); }); bindAction('exportTramoCsvBtn','exportTramo',exportTramoCsv); bindAction('copySummaryBtn','copySummary',copySummary); bindAction('copyChatgptBtn','copyChatgpt',copyChatgptSummary); bindAction('exportPlanTxtBtn','exportPlan',exportPlanTxt); bindAction('clearAppDataBtn','clearAppData',clearAppData,500);
  ...
  loadSettings(); bind(); runStartupDiagnostics(); registerPWA(); updateSavedSessionInfo(); updateExtraSavedInfo(); renderHistory(); updateAppStatus(); renderSettingsUI();
