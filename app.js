...
(() => {
  ...
  const state = {
    ...
    settings: { browserNotify:false, voiceAlerts:true, beepAlerts:true, voiceName:'', voiceVolume:1, voiceRate:1 },
    alerts: { fired:{}, queue:[], busy:false, audioCtx:null }
  };
  ...
  function queueSegmentCues(){
    ...
    const t = machineElapsed();
    const current = buildSegmentWindows().find(w => t >= w.start && t < w.end - 1e-9);
    if (!current) return;
    const next = nextWindowAfterCurrent();
    ...
    speechThresholds.forEach(th => {
      if (remainSeg === th){
        let text = '';
        if (th >= 60) text = 'Quedan ' + Math.round(th/60) + ' minuto' + (th>=120?'s':'') + ' de tramo. Próximo ' + (isTestPlan() && next && next.seg === 'B' ? 'test ' : '') + changeKind + ' a ' + nextLabel + '.';
        else text = 'Quedan ' + th + ' segundos de tramo. Próximo ' + (isTestPlan() && next && next.seg === 'B' ? 'test ' : '') + changeKind + ' a ' + nextLabel + '.';
        enqueueCue({ key:'seg-' + current.seg + '-' + th, type:'speech', text, notifyTitle:'Cambio de tramo', notifyBody:text });
      }
    });
    [3,2,1].forEach(th => { if (remainSeg === th) enqueueCue({ key:'segbeep-' + current.seg + '-' + th, type:'beep', count:1 }); });
    const nextWater = state.plan.waters.find((w,i)=>!state.plan.waterTaken[i] && w > t + 1e-9);
    if (Number.isFinite(nextWater)){
      ...
      [3,2,1].forEach(th => { if (remainWater === th) enqueueCue({ key:'waterbeep-' + nextWater + '-' + th, type:'beep', count:1 }); });
    }
    if (remainSeg > 0 && remainSeg % 120 === 0) enqueueCue({ key:'segremain-' + current.seg + '-' + remainSeg, type:'speech', text:'Quedan ' + fmtTime(remainSeg) + ' de este tramo.' });
    if (remainAll > 0 && remainAll % 300 === 0) enqueueCue({ key:'allremain-' + remainAll, type:'speech', text:'Quedan ' + fmtTime(remainAll) + ' del ejercicio.' });
  }
  ...
  function runStartupDiagnostics(){
    ...
    const fnMap = {withTapGuard, bindAction, drawCompareChart, refreshAll, toggleRun, applyImport, previewImport, normalizeCurrentImport, refreshStatusStrip, renderHistory, refreshSessionAlert, exportTramoCsv, updateAppStatus, copySummary, copyChatgptSummary, exportPlanTxt, clearAppData, compareLastAction, finalSummaryAction, requestBrowserNotifications, saveSettings, loadSettings, renderSettingsUI, queueSegmentCues}; let ok=0, fail=0;
